...
dependencies {
    ...
    implementation ("com.skplanet.sdk.ad:skpad-benefit:1.14.9")
     ...생략...
}
```
...
다음은 광고 할당 후 Feed 지면을 표시하는 예시입니다. 다음과 같이 구현하면 사용자가 Feed 지면에 진입하기 전에 광고 할당이 보장되기 때문에 "참여할 수 있는 광고가 없습니다."는 이미지를 보이지 않습니다.<br>
App에서는 광고의 존재 여부를 체크하여, Feed 지면을 실행 여부와 진입 메뉴를 노출을 결정할 수 있습니다.
...
feedHandler.preload(new FeedHandler.FeedPreloadListener() {
    ...
    public void onPreloaded() {
        ...
        // 광고의 존재 여부를 체크하여, Feed를 실행하고나 메뉴를 노출합니다.
        feedHandler.startFeedActivity(context);
    }
...
dependencies {
    implementation ("com.skplanet.sdk.ad:skpad-benefit:1.14.9") { changing = true }  // SKP AD Benefit SDK 라이브러리
}
```
